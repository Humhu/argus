...
bool GetParam<FullNumericRange, ros::NodeHandle>( const ros::NodeHandle& src,
                                                  ...
                                                  FullNumericRange& t )
{
	if( !GetParam( src, name + "/min", t.min ) )
	{
		ROS_WARN_STREAM( "Could not read: " + name + "/min" );
		return false;
	}
	if( !GetParam( src, name + "/init", t.init ) )
	{
		ROS_WARN_STREAM( "Could not read: " + name + "/init" );
		return false;
	}
	if( !GetParam( src, name + "/max", t.max ) )
	{
		ROS_WARN_STREAM( "Could not read: " + name + "/max" );
		return false;
	}
	return true;
}
